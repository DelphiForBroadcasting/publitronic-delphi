...
type
  TSyntaxElement = (seSymbol, seReservedWord, seString, seNumber, seDirective,
    seComment,seReservedAttributes);
...
const
  ElementSettings: array [TSyntaxElement] of TElementSettings = (
    ...
    (Color: clred; Style: []));              // attributes

const
  ReservedWordsAndDirectives: array [0..7] of String = ('xml','window','label','textfield','unitext','combobox','TEXTAREA','DROPFIELD');
  ReservedAttribute: array [0..7] of String = ('name','x','y','width','height','id','max','search');

var
  ReservedWords: TStringList = nil;
  ReserveAttributes: TStringList = nil;
...
begin
  ...
  ReserveAttributes := TStringList.Create;
  ReserveAttributes.Sorted := True;
  ReserveAttributes.Duplicates := dupError;

  for S in ReservedWordsAndDirectives do  ReservedWords.Add(S);

  for S in ReservedAttribute do  ReserveAttributes.Add(S);
end;

procedure Finalize;
begin
  FreeAndNil(ReservedWords);
  FreeAndNil(ReserveAttributes);
end;
      ...
      '_', 'A'..'Z', 'a'..'z':
        begin
          ...
          if (ReservedWords.IndexOf(Q) >= 0) then
            Add(seReservedWord, Q) else
          if (ReserveAttributes.IndexOf(Q) >= 0) then
            Add(seReservedAttributes, Q)
          else
            Add(seSymbol, Q);
          P^ := C;
        end;
